...
class User {
    ...
    public function tampilRincianRequestBarang($id){
        $id=$_SESSION['user_id'];
        ...
        FROM peminjaman as p inner join list_barang as lb on p.id_peminjaman=lb.id_peminjaman inner join barang as b on b.id_barang=lb.id_barang WHERE p.user_id='$id' and p.status='request' and p.id_peminjaman='$id';";
        $this->db->query($querry);
        return $this->db->resultSet();
    }
    public function tampilSemuaPeminjaman(){
        ...
        WHERE p.status = 'request' AND p.user_id=$id
        UNION
    ...
    public function tampilProfile(){
        ...
        if($level=='mahasiswa'){
        $query = "SELECT mhs.nama_mhs AS nama, mhs.nim AS nim, mhs.jk AS jk, u.username AS username, u.email AS email, u.level as level FROM user AS u 
        INNER JOIN mahasiswa AS mhs ON ,hs.nim = u.unicode WHERE u.level = '$level' AND u.user_id = '$id'";
        $this->db->query($query);
        return $this->db->single();
        }else{
            $query = "SELECT mh.nama_mhs AS nama, mhs.nim AS nim, mhs.jk AS jk, u.username AS username, u.email AS email, u.level as level FROM user AS u 
            INNER JOIN mahasiswa AS mhs ON ,hs.nim = u.unicode WHERE u.level = '$level' AND u.user_id = '$id'";
            $this->db->query($query);
            return $this->db->single();
        }
